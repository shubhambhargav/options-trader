...
class KiteTickerClientProtocol(WebSocketClientProtocol):
    ...
    def onPong(self, response):  # noqa
        ...
        if self._last_pong_time and self.factory.debug:
            LOGGER.debug("last pong was {} seconds back.".format(time.time() - self._last_pong_time))
    ...
    def _loop_ping(self): # noqa
        ...
        if self.factory.debug:
            ...
            if self._last_ping_time:
                LOGGER.debug("last ping was {} seconds back.".format(time.time() - self._last_ping_time))
...
class KiteTickerClientFactory(WebSocketClientFactory, ReconnectingClientFactory):
    ...
    def clientConnectionFailed(self, connector, reason):  # noqa
        ...
        if self.retries > 0:
            LOGGER.error("Retrying connection. Retry attempt count: {}. Next retry in around: {} seconds".format(self.retries, int(round(self.delay))))
    ...
    def send_noreconnect(self):
        ...
        if self.maxRetries is not None and (self.retries > self.maxRetries):
            if self.debug:
                LOGGER.debug("Maximum retries ({}) exhausted.".format(self.maxRetries))
...
class KiteTicker(object):
    ...
    def __init__(self, user_id, enctoken, debug=False, root=None,
        ...
        self.socket_url = 'wss://ws.zerodha.com/?api_key=kitefront&user-agent=kite3-web&version=2.9.3&user_id=%(user_id)s&enctoken=%(enctoken)s' % {
            'user_id': user_id,
            'enctoken': enctoken
        }
    ...
    def resubscribe(self):
        ...
        for mode in modes:
            if self.debug:
                LOGGER.debug("Resubscribe and set mode: {} - {}".format(mode, modes[mode]))
